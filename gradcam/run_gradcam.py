#!/usr/bin/env python3
"""
CLI driver for the Grad-CAM cross-architecture visualization: per sample, CNN/VAE image Grad-CAM on
layer4 (overlay PNG + .npy) and ShapeEmbed contour-point saliency (outline PNG), plus mask-containment,
per-body-third CAM mass, and CNN-vs-VAE agreement metrics.
...
def crop_mask(mask_path, size=352, crop=1536, ref_shape=None):
    """Full-res mask -> center-crop -> resize (nearest). Returns None if the mask shape != image (ref_shape)."""
    from skimage.io import imread
...
def place_crop_in_full(cam_crop, native_shape, out_hw, crop=1536):
    """Map a CAM computed on the central crop of a native-size image back into the full frame at out_hw,
    at the crop's true footprint (zero outside)."""
    from skimage.transform import resize
    H0, W0 = native_shape
...
def prep_vae_fullres(image_path, size=352, crop=1536):
    """VAE inference transform: full-res read -> CenterCrop(crop) -> Resize(size) -> per-image standardization."""
    import torch
...
def load_split_for_fit(csv, label_col, split, split_col, mask_dir, mask_col, mask_ext,
    ...
    """Loader for head fitting: images always, masks best-effort (zero if missing).
    Returns (imgs, masks, labels[int], paths)."""
    import pandas as pd
    from skimage.io import imread
    from skimage.transform import resize
    df = pd.read_csv(csv)
    ...
    print(f"[train-fit] loaded {len(imgs)} {split} samples ({n_mask} with masks) for head fitting")
...
def top_mass(cam, frac=0.2):
    """Keep pixels carrying the top `frac` of total CAM mass; rest -> NaN (transparent in imshow)."""
...
def save_heatmap(image, cam, path, title, mask=None, top_frac=0.2):
    ...
    if mask is not None and np.asarray(mask).sum() > 0:                 # draw the sample outline
...
def save_contour(image, pts, sal, path, title, top_frac=0.2):
    ...
    hot = sal >= thr
    ax.scatter(pts[hot, 1], pts[hot, 0], c=sal[hot], cmap='jet', s=18, vmin=0, vmax=1)  # no colorbar
...
def cam_metrics(cam, mask):
    """CAM localization vs the sample mask. Reports containment, background mass, and enrichment =
    (energy fraction in a region) / (its area fraction): >1 attends more than chance, <1 less."""
...
def extract_feature(spec, image, mask, path, device):
    """Re-extract the penultimate feature via the same forward pass Grad-CAM uses (no grad)."""
    import torch
    if spec['kind'] == 'image':
...
def _train_spearman(head, classes, X, y):
    """Sanity check: Spearman between E[k] and the label on the train features."""
    import torch
...
def build_backbones(methods, args, device):
    """Build the per-method backbone specs (kind, model, target_layer, feature_fn, prep)."""
    bb = {}
    if 'CNN' in methods:
    ...
    if 'VAE' in methods:
        ...
        # VAE reads full-res with CenterCrop(1536)->Resize(352), so its CAM lives in the 352 crop frame.
...
def main():
    ...
    ap.add_argument('--subset-file', default='../occlusion/results_occlusion/occlusion_subset_ids.txt',
                    help='reuse the frozen subset from occlusion (default points at it)')
    ap.add_argument('--n-vis', type=int, default=3, help='overlays saved per class (metrics use all)')
    ...
    ap.add_argument('--vae-crop', type=int, default=1536,
                    help="VAE full-res CenterCrop size before resize")
    ...
    if not imgs:
        raise SystemExit("[ERR] no samples loaded -- check --csv / --mask-dir / --mask-ext / --subset-file")
    labels = meta[a.label_col].astype(int).values if a.label_col in meta.columns else np.zeros(len(imgs), int)
    data_ids = meta['data_id'].values if 'data_id' in meta.columns else np.arange(len(imgs)).astype(str)

    bb = build_backbones(a.methods, a, device)

    # ---- class heads fit on re-extracted train features. CNN/VAE ignore the mask; ShapeEmbed needs it. ----
    ...
    for name in a.methods:
        ...
        if len(X) >= 20 and len(set(y)) >= 2:
            ...
            flag = 'OK' if rho >= 0.3 else 'LOW -> extraction may lose the class signal'
            print(f"[head:{name}] re-extracted {len(X)} train features  "
                  f"train Spearman(E[k],label)={rho:.2f}  {flag}")
    ...
    rows = []
    image_cams = {}                                                  # (data_idx, method) -> cam for cross-corr
    for i, (img, msk) in enumerate(zip(imgs, msks)):
        y = int(labels[i]); fid = str(data_ids[i]); pth = str(paths[i])
    ...
    if 'CNN' in a.methods and 'VAE' in a.methods:
        ...
        for i, msk in enumerate(msks):
            if (i, 'CNN') in image_cams and (i, 'VAE') in image_cams:
                corr.append(dict(fish=str(data_ids[i]), SC=int(labels[i]),
    ...
    if len(summ):
        ...
        print('\n[summary] mean CAM containment / enrichment (=containment/area, >1 attends to sample) '
